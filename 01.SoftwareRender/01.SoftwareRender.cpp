// 01.SoftwareRender.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
void MyGameHost::GameInit()
{
   ...
   */
	...
	cubeNode->SetVertexColor(RED_F);
	cubeNode->SetShaderType(GOURAUD_SHADER);
	...
	cubeNode2->SetVertexColor(BLUE_F);
	cubeNode2->SetShaderType(GOURAUD_SHADER);
	...
	cubeNode3->SetVertexColor(YELLOW_F);
	cubeNode3->SetShaderType(FLAT_SHADER);

	CubeSceneNode* cubeNode4 = CreateCubeNode(
		Vector4(-1000, -1000, 50, 1),
		Vector3(1, 1, 1),
		Vector3(0, 150, 0));
	cubeNode4->SetRenderState(Z_BUFFER, renderState[Z_BUFFER]);
	cubeNode4->SetRenderState(FLAT, true);
	cubeNode4->SetRenderState(RENDER_TRIANGLE, renderState[RENDER_TRIANGLE]);
	cubeNode4->SetRenderState(ON_MOVE_BACKFACE, renderState[ON_MOVE_BACKFACE]);
	cubeNode4->SetShaderType(FLAT_SHADER);

	AddRenderStateNodes(cubeNode);
	AddRenderStateNodes(cubeNode2);
	AddRenderStateNodes(cubeNode3);
	AddRenderStateNodes(cubeNode4);
